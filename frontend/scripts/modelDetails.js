    ...
    const loadModelDetails = async (collectionId, itemId) => {
        try {
            ...
            const renderStacExtensions = (extensions) => {
                if (!extensions || extensions.length === 0) return '<p>No extensions available.</p>';
                return extensions
                    .map(ext => `<a href="${ext}" target="_blank" rel="noopener noreferrer">${ext}</a>`)
                    .join(', ');
            };

            // Render Classification Classes with Color Boxes
            const renderClassificationClasses = (classes) => {
                if (!classes || classes.length === 0) return '<p>No classification data available.</p>';
                return `
                    <div class="classification-classes">
                        ${classes.map(cls => `
                            <div class="class-item">
                                <div class="color-box" data-color="#${cls.color_hint || 'FFFFFF'}"></div>
            ...
            modelDetailsContainer.innerHTML = `
                <div class="model-details-container">
                    <div class="text-section">
                        ...
                        ${createDetailRow('STAC Extensions', renderStacExtensions(model.stac_extensions))}
